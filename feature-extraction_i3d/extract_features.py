...
def load_frame(frame_file, resize=False):
    ...
    assert(data.size[1] == width)
    assert(data.size[0] == height)
...
def load_rgb_batch(frames_dir, rgb_files, 
    ...
    if resize:
        ...
        batch_data = np.zeros(frame_indices.shape + (width,height,3))
...
def load_flow_batch(frames_dir, flow_x_files, flow_y_files, 
    ...
    if resize:
        ...
        batch_data = np.zeros(frame_indices.shape + (width,height,2))
